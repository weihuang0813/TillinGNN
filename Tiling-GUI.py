...
from lego.smooth import nearest_of_points, center_move
import time
SIMPLIFY_RATIO = 5e-3
scale_ratio = 0.02
...
class DrawerWidgets(QMainWindow):
    def __init__(self, graph, debugger : MyDebugger, plotter : Plotter, solver, app : QApplication):
        ...
        self.current_polygon_exterior_ldr = []
        self.current_polygon_interiors_ldr = []
        self.original_exterior = []
        self.filename = ""
        self.scale = None
...
######################### KEY PRESS ########################################
    def keyPressEvent(self, event):
        ...
        if type(event) == QKeyEvent:
            ...
            if event.key() == ord('E'):
                ...
                x_divide_move, y_divide_move = center_move(self.current_polygon_exterior[0], 0.25, 0.3)
                self.affline_transform(translation_x_rate = x_divide_move)
                self.affline_transform(translation_y_rate = y_divide_move)
    ...
    def affline_transform(self, scale_rate = None, translation_x_rate = None, translation_y_rate = None, rotation_angle = None):
        ...
        base_diameter, target_polygon = factory.shape_transform(self.complete_graph, self.current_polygon_exterior, self.current_polygon_interiors,
                                                                        scale_rate, rotation_angle, translation_x_rate,
                                                                        translation_y_rate)
        '''
        self.repaint()
...
######################### PAINT ########################################
    ...
    ######################### OTHERS ########################################
    def deformation(self, accept_ratio = None):
        if accept_ratio is None:
            accept_ratio, ok = QInputDialog.getDouble(self, "How close does it need to be to the closest point ?", "Enter a ratio 0 to 100")
        self.current_polygon_exterior = nearest_of_points(self.current_polygon_exterior, accept_ratio)
        self.repaint()

    def open_silhouette_files(self, margin_padding_ratio = None, rotate_angle = None, x_delta = None, y_delta = None, compare_use = None, trans_state =None):
        ...
        if compare_use is None:
            compare_use, ok = QInputDialog.getDouble(self, "Is this use to compare the result ?", "0:false, not 0:true")
        ...
        if trans_state is None:
            trans_state, ok = QInputDialog.getDouble(self, "Enter state ", "Enter a transform state \n 0 : false \n 1 : true")
        
        if margin_padding_ratio is not None and \
            rotate_angle is not None and \
            x_delta is not None and \
            y_delta is not None and \
            trans_state is not None:
            ok = True
        ...
        if os.path.exists(filename) and ok:
            ...
            if(not compare_use):
                ...
                target_polygon = Polygon(exterior_contour, holes=interior_contours)
    ...
    def compute_Brick_Layout(self):
        ...
        # print(self.current_polygon_exterior)
        # print(self.current_polygon_interiors)
        self.current_polygon_exterior_ldr = self.current_polygon_exterior
        self.current_polygon_interiors_ldr = self.current_polygon_interiors

        for i in range(len(self.current_polygon_exterior)):
            for j in range(len(self.current_polygon_exterior[0])):
                self.current_polygon_exterior_ldr[i][j] = self.current_polygon_exterior_ldr[i][j] * 80
        for i in range(len(self.current_polygon_interiors)):
            for j in range(len(self.current_polygon_interiors[i])):
                self.current_polygon_interiors_ldr[i][j] = self.current_polygon_interiors_ldr[i][j] * 80
        export_contour_as_text("unity_use/contour.txt", (self.current_polygon_exterior_ldr, self.current_polygon_interiors_ldr))
        
        ###
        #為何明明沒動到self.current_polygon_exterior,self.current_polygon_interiors卻不同？
        for i in range(len(self.current_polygon_exterior)):
            for j in range(len(self.current_polygon_exterior[0])):
                self.current_polygon_exterior[i][j] = self.current_polygon_exterior[i][j] / 80
        for i in range(len(self.current_polygon_interiors)):
            for j in range(len(self.current_polygon_interiors[i])):
                self.current_polygon_interiors[i][j] = self.current_polygon_interiors[i][j] / 80
        ###
        ...
        if ok:
            ...
            for i in range(trial_times):
                ...
                if score != 0:
                    ...
                    QQ = result_brick_layout.show_predict_for_ldr(plotter, self.debugger, self.filename, False, os.path.join(save_path,
                                                                           f'{score}_{self.brick_layout_cnt}_{self.solution_cnt}_predict.png') )
                    time_end = time.time()    #結束計時
                    time_c= time_end - time_start   #執行所花時間
                    print('merge-time cost', time_c, 's')

                    time_end2 = time.time()    #結束計時
                    time_c2= time_end2 - time_start2   #執行所花時間
                    print('all-time cost2', time_c2, 's')
            ...
            target_polygon = Polygon(self.current_polygon_exterior, holes=self.current_polygon_interiors)
            coverage = result_brick_layout.get_super_contour_poly().area / target_polygon.area
            print("##################")
