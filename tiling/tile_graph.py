...
def rotate_judgment(base_tile: Tile, base_edge_idx, align_tile: Tile, align_edge_idx, align_mode):
    base_edge_p0, base_edge_p1 = base_tile.get_edge(base_edge_idx)
    R = tiling_util.align_tile_judge(base_edge_p1, base_edge_p0, align_tile.tile_poly, align_edge_idx, align_mode)
    if(abs(R[0, 0] - 1) > EPS): return True
    if(abs(R[0, 1] - 0) > EPS): return True
    if(abs(R[1, 0] - 0) > EPS): return True
    if(abs(R[1, 1] - 1) > EPS): return True
    return False

def get_all_tiles(base_tile: Tile, align_tile: Tile, integer_align):
    #可透過 get_edge_num(), get_perimeter() 和 area() 來判斷圖塊的種類
        ...
        for align_neighbor_idx in range(align_tile.get_edge_num()):
            for align_mode in [0, 1]:
                ...
                if integer_align:
                    ...
                    judge = rotate_judgment(base_tile, base_edge_idx, align_tile, align_neighbor_idx, align_mode=align_mode)
                    
                    # 排除旋轉過後的2x1x2的斜磚60477
                    if(1.8 < align_tile.get_perimeter() and align_tile.get_perimeter() < 1.9 and judge == False):
                        continue

                    # 排除旋轉過後的2x1x2的斜磚60481
                    if(1.4 < align_tile.get_perimeter() and align_tile.get_perimeter() < 1.6 and judge == False):
                        continue
                    # 排除旋轉過後的1x1x2/3的斜磚54200
                    if(align_tile.get_perimeter() < 0.8  and judge == False):
                        continue
                    if(round(base_edge_length,2) == 0.2 or round(tile_edge_length,2) == 0.2):
                        continue

                    # 排除所有斜磚（除54200以外）與下方磚的連結
                    if(base_tile.get_perimeter() > 0.8 and align_tile.get_perimeter() > 0.8):
                        if(base_tile.get_edge_num() == 3):
                            if(round(base_edge_length,2) == slope_b1 or round(base_edge_length,2) == slope_b2 or round(base_edge_length,2) == slope_b3):
                                continue
                        if(align_tile.get_edge_num() == 3):
                            if(round(tile_edge_length,2) == slope_b1 or round(tile_edge_length,2) == slope_b2 or round(tile_edge_length,2) == slope_b3):
                                continue 

                    # 排除斜磚左右相鄰
                    if base_tile.get_edge_num() == 3 and align_tile.get_edge_num() == 3 :
                        if(round(base_edge_length,2) == slope_1 or round(base_edge_length,2) == slope_2 or round(base_edge_length,2) == slope_3):
                            if(round(tile_edge_length,2) == slope_1 or round(tile_edge_length,2) == slope_2 or round(tile_edge_length,2) == slope_3):
                                continue
                    ...
                    # 排除斜邊（須修改成通用斜邊）
                    # if base_tile.get_edge_num() == 3:...
                    if 0.33 > base_edge_length and base_edge_length > 0.31 :  
                        continue
                    if 0.33 > tile_edge_length and tile_edge_length > 0.31 :  
                        continue
                    if 0.4 > base_edge_length and base_edge_length > 0.35 :  
                        continue
                    if 0.4 > tile_edge_length and tile_edge_length > 0.35 :  
                        continue
                    if 0.6 > base_edge_length and base_edge_length > 0.55 :  
                        continue
                    if 0.6 > tile_edge_length and tile_edge_length > 0.55 :  
                        continue
                    if 0.7 > base_edge_length and base_edge_length > 0.6 :  
                        continue
                    if 0.7 > tile_edge_length and tile_edge_length > 0.6 :  
                        continue
                    if 0.81 > base_edge_length and base_edge_length > 0.8 :  
                        continue
                    if 0.81 > tile_edge_length and tile_edge_length > 0.8 :  
                        continue
                    if 0.95 > base_edge_length and base_edge_length > 0.92 :  
                        continue
                    if 0.95 > tile_edge_length and tile_edge_length > 0.92 :  
                        continue
