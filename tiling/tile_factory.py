...
def crop_multiple_layouts_from_contour_offset(exterior_contour, interior_contours, complete_graph, count):
    print("cropping from super set...")
    result_brick_tuples = []
    
    margin_padding_ratio = 0.65
    rotate_angle = 0

    for i in range(count):
        x_delta = round(random.random(),2)
        y_delta = round(random.random(),2)
        print(f"x:{x_delta}")
        print(f"y:{y_delta}")
        base_diameter, target_polygon = shape_transform(complete_graph,
                                                        exterior_contour,
        ...
        print("###########################")
        temp_poly = list(target_polygon.exterior.coords)
        print(temp_poly)
        print(temp_poly[0])
        print(temp_poly[0][0])
        print("$$$###########################$$$")
        ...
        # deformation and divide
        if(config.deformation_ratio != 0 and config.deformation == True):
            nearest_of_points(target_polygon, config.deformation_ratio)
...
def shape_transform(complete_graph, exterior_contour, interior_contours, margin_padding_ratio, rotate_angle, x_delta,
    ...
    if(trans_state == 1):
        ...
        exterior_contour_resized = exterior_contour
        interior_contour_resized = [interior_contour for interior_contour in interior_contours]
    
    scale_ratio = (1/max_axis) * base_diameter * margin_padding_ratio
    print("scale ratio =" ,scale_ratio) # 控制這行不要執行
    print("trans state is ", trans_state)

    input_polygon = Polygon(exterior_contour_resized, holes=interior_contour_resized)
    input_polygon_centroid = np.array(input_polygon.centroid)# 非零像素的質心
    # print(input_polygon_centroid)
    # print(input_polygon)
